...
public class LinkedControlFrequencySlot extends ValueBoxTransform.Dual {

    public LinkedControlFrequencySlot(boolean first) {
        super(first);
    }
    ...
    public Vec3 getLocalOffset(LevelAccessor level, BlockPos pos, BlockState state) {
        ...
        if(facing.getAxis().isHorizontal()) {
            return rotateHorizontally(state, isFirst()
                    ? VecHelper.voxelSpace(6.2f, 4.5f, 3.125f)
                    : VecHelper.voxelSpace(9.8f, 4.5f, 3.125f)
            );
        }

        return VecHelper.rotateCentered(isFirst()
                ? VecHelper.voxelSpace(9.8f, 3.15f, 4.5f)
                : VecHelper.voxelSpace(6.2f, 3.15f, 4.5f)
