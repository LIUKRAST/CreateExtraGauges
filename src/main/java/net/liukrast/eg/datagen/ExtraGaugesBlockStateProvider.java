...
public class ExtraGaugesBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        horizontalFaceBlock(EGBlocks.ROSE_QUARTZ_PORT.get(), state -> {
            if(state.getValue(RedstonePortBlock.OUTPUT) && state.getValue(RoseQuartzPortBlock.POWER) > 0) return models().getExistingFile(EGConstants.id("block/rose_quartz_port/on_out"));
            if(state.getValue(RedstonePortBlock.OUTPUT) && state.getValue(RoseQuartzPortBlock.POWER) == 0) return models().getExistingFile(EGConstants.id("block/rose_quartz_port/off_out"));
            if(!state.getValue(RedstonePortBlock.OUTPUT) && state.getValue(RoseQuartzPortBlock.POWER) > 0) return models().getExistingFile(EGConstants.id("block/rose_quartz_port/on_in"));
